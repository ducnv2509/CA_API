...
router.post('/insertTicket/',validateTokenCustomerAccess,  async (req, res,next) => {
    let { category_id, email, resolved_date, description } = req.body;
    let {username} = req.payload;
    myLogger.info('%o', req.payload);
    let response = await insertTicketByCustomer(username, category_id, email, resolved_date, description);
    next(response);
})
